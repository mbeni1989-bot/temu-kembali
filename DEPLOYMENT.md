...
## 🔗 Konfigurasi GitHub → Railway (Auto Deploy)

Agar setiap **push ke GitHub** otomatis **build dan deploy** di Railway:

1. **Login Railway** → https://railway.app  
2. **New Project** → pilih **"Deploy from GitHub repo"**.  
3. **Connect GitHub** (jika belum):
   - Klik **"Configure GitHub App"** / **"Connect GitHub"**.
   - Pilih **organization** atau **akun**, lalu **authorize** repo yang dipakai (mis. `mbeni1989-bot/temu-kembali` atau repo Anda).
4. **Pilih repository** → pilih repo Temu Kembali.  
5. **Pilih branch** (biasanya `main` atau `master`) → deploy akan jalan tiap push ke branch ini.  
6. Railway akan **clone repo**, baca `railway.json` / `nixpacks.toml`, lalu **build & deploy** otomatis.

Setelah itu: **setiap push/merge ke branch yang terhubung = build baru + deploy otomatis**. Tidak perlu deploy manual kecuali mau trigger dari dashboard.
...
4. Choose repository: **`mbeni1989-bot/temu-kembali`** (atau repo Anda)
...
# Mapbox Token (dapatkan di https://account.mapbox.com/)
